...
# -*- coding: utf-8 -*-
"""
BTC regressions FROM PRICE LEVELS, applying transforms dictated by the () suffix.
...
(e.g., "NKYTR Index (Natural Log Changes)"), we still treat that header as the
LEVEL column and then apply the specified transform to it.

Output:
  BTC_regressions.tex   (LaTeX table per sheet, stepwise specs)
"""
...
OVERRIDES = {
    # "SPXT Index": "SPXT Index",  # add entries only if a sheet has a funky header
}
...
# Pretty labels for LaTeX rows (BTC)
label_map = {
...
def resolve_level_col(df: pd.DataFrame, desired_level_name: str, original_with_suffix: str) -> str:
    ...
    Priority:
      ...
      2) Fallback: the suffixed header itself (your workbook may use these as LEVEL headers)
...
def run_sheet(sheet_name: str, spec: dict, writer):
    ...
    lines.append(f"\\caption{{Bitcoin {sheet_name}}}")
...
if __name__ == "__main__":
    with open("BTC_regressions.tex", "w") as f_out:
        for sheet, spec in regression_specs.items():
            run_sheet(sheet, spec, f_out)
    print("All BTC regressions completed FROM LEVELS. Results saved to BTC_regressions.tex")
