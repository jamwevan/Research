...
# -*- coding: utf-8 -*-
"""
Litecoin regressions FROM PRICE LEVELS, applying transforms dictated by the () suffix.

Rules:
  "(Natural Log Changes)" -> Δlog(level)  [decimals]
  "(Changes)"              -> Δlevel

This script NEVER uses pre-transformed data. If a header already includes the suffix
...
Output:
  LTC_regressions.tex   (LaTeX table per sheet, stepwise specs)
"""
...
OVERRIDES = {
    # "SPXT Index": "SPXT Index",  # add only if a sheet has a quirky header
}
...
# Pretty labels for LaTeX rows (LTC)
label_map = {
    "LTC_USD (Natural Log Changes)": r"$\Delta \log(\mathrm{LTC}_{USD})$",
    "LTC_GBP (Natural Log Changes)": r"$\Delta \log(\mathrm{LTC}_{GBP})$",
    "LTC_EUR (Natural Log Changes)": r"$\Delta \log(\mathrm{LTC}_{EUR})$",
    "LTC_JPY (Natural Log Changes)": r"$\Delta \log(\mathrm{LTC}_{JPY})$",
    "LTC_CNY (Natural Log Changes)": r"$\Delta \log(\mathrm{LTC}_{CNY})$",
    "LTC_BRL (Natural Log Changes)": r"$\Delta \log(\mathrm{LTC}_{BRL})$",
    "LTC_INR (Natural Log Changes)": r"$\Delta \log(\mathrm{LTC}_{INR})$",
    "LTC_ZAR (Natural Log Changes)": r"$\Delta \log(\mathrm{LTC}_{ZAR})$",
...
def clean_name(s: str) -> str:
    """Lowercase, strip, collapse spaces (keeps vendor suffixes if present)."""
    s = str(s).replace("\u00A0", " ")
...
def resolve_level_col(df: pd.DataFrame, desired_level_name: str, original_with_suffix: str) -> str:
    ...
    Priority:
      ...
      2) Fallback: the suffixed header itself (some sheets use these as the LEVEL header)
      3) Fuzzy match (>=0.85) of the base name
    """
    # Manual override
    if desired_level_name in OVERRIDES and OVERRIDES[desired_level_name] in df.columns:
        return OVERRIDES[desired_level_name]
...
def run_sheet(sheet_name: str, spec: dict, writer):
    ...
    lines.append(f"\\caption{{Litecoin {sheet_name}}}")
...
if __name__ == "__main__":
    with open("LTC_regressions.tex", "w") as f_out:
        for sheet, spec in regression_specs.items():
            run_sheet(sheet, spec, f_out)
    print("All LTC regressions completed FROM LEVELS. Results saved to LTC_regressions.tex")
