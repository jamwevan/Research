...
XLSX = BASE_DIR / "For New Prepayment Research.xlsx"
OUT_DIR = BASE_DIR / "exported_images"
...
def parse_workbook_sheets(zf: zipfile.ZipFile):
    """
    Returns list of (sheet_name, sheet_target) where sheet_target is like 'worksheets/sheet1.xml'
    in workbook order.
    """
    ...
    rid_to_target = {rel.attrib["Id"]: rel.attrib["Target"] for rel in rels.findall("rel:Relationship", rns)}
...
def drawing_path_for_sheet(zf: zipfile.ZipFile, sheet_target: str):
    """
    For a sheet xml target 'worksheets/sheetN.xml', find its drawing xml path 'xl/drawings/drawingX.xml'.
    """
...
def images_for_drawing(zf: zipfile.ZipFile, drawing_path: str):
    ...
    # drawing rels: map rId -> target media file
    ...
    with zipfile.ZipFile(XLSX, "r") as zf:
        ...
        for sheet_name, sheet_target in sheets:
            ...
            for i, (_r, _c, media_path) in enumerate(anchors, start=1):
                ...
                base = Path(media_path).name  # e.g., image123.png
