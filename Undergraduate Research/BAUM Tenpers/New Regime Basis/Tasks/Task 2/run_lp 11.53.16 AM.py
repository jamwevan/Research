...
def run_lp(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for n in MATURITIES:
        for h in HORIZONS:
            ...
            #
            # IMPORTANT (per Oyakhi): use NON-CUMULATIVE responses.
            # Instead of the cumulative level change x_{t+h} - x_{t-1},
            # we want the h-step increment/difference:
            #   Δx_{t+h} := x_{t+h} - x_{t+h-1}.
            # In LP notation, the dependent variable is:
            #   y^{(h)}_{n,t} = x_{n,t+h} - x_{n,t+h-1}.
            x_col = f"x{n}"
            require_columns(df, [x_col] + X_COLS)

            # For h=0 this is Δx_t = x_t - x_{t-1} (since shift(-(h-1)) = shift(+1)).
            y_series = (
                df[x_col].astype(float).shift(-h)
                - df[x_col].astype(float).shift(-(h - 1))
            )
            sub = df[X_COLS].copy()
...
def write_latex_tables(res: pd.DataFrame, outdir: Path) -> None:
    ...
    for n in MATURITIES:
        ...
        lines.append(
            ...
            r"Dependent variable is the non-cumulative change "
            r"$\Delta x_{t+h}:=x_{t+h}-x_{t+h-1}$ for the "
            + f"{n}Y"
...
def write_irf_plots(res: pd.DataFrame, outdir: Path) -> None:
    ...
    for n in MATURITIES:
        ...
        plt.ylabel(r"Non-cumulative IRF: coef on $\Delta D$")
        plt.title(f"Regime-conditional local projections (non-cumulative, JPY basis {n}Y)")
        plt.legend()
